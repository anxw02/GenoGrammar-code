"""
FORMAL MATCHED BASELINE — cnn1d / R4 ContextSSL [cnn1d]
...
_BASELINE_NAME = 'cnn1d'
_BASELINE_CLASS = _BASELINE_REGISTRY[
    _BASELINE_NAME
]

_EXPECTED_BASELINE_PARAMS = 508310
# ============================================================
...
def main():
    ...
    if definition_file.exists():
        ...
        for key in critical:
            ...
            #
            # Python uses integer fold IDs, while JSON object keys
            # are restored as strings. Seed VALUES remain exact.
            # All other frozen fields retain exact comparison.
            # ----------------------------------------------------
    ...
    for fold in FOLDS:
        ...
        # RESUME-SAFE FOLD 1
        #
        # Fold 1 was already formally trained and outer-evaluated
        # under the same frozen protocol before the unrelated
        # lineage-aware outer-size guard was corrected for Fold 2.
        #
        # DO NOT call train_fold() for Fold 1: train_fold() deletes
        # existing fold artifacts at entry.
        #
        # Reuse is permitted ONLY if all five previously frozen
        # artifacts match their exact SHA256 values.
        # ============================================================
        ...
        if fold == 1:
            ...
            resume_expected_sha = {
                "DONE.json":
                    "5902ab51baa4877c165805444c8366468af0339ed5b3ed95b7ac76bee65defdf",

                "best_checkpoint.pt":
                    "729ee216d218b7b480dc9bf5cf6ec7989d3d43b392b7bc26ddfd77a2b44866f3",

                "outer_metrics.json":
                    "322b754fc0437f3ae548415e48a35cbb798402f53d9c6411e2775724e2fdad27",

                "split_manifest.csv":
                    "d46919e44b1a9551ef104b9a9e1084394130ce8c5f0b566d6143b19d3d5b2720",

                "training_history.csv":
                    "bec439ac7275f7acfd8a8aab527d01cfe84587f63d9c729874495c7adcfd100c",
            }


            resume_errors = []

            for resume_name, resume_sha in (
                resume_expected_sha.items()
            ):
                ...
                if resume_hash != resume_sha:

                    resume_errors.append(
                        (
                            f"sha256:{resume_name}:"
                            f"{resume_hash}"
                        )
                    )
            ...
            if resume_errors:
                ...
                raise RuntimeError(
                    "Fold 1 frozen resume verification failed: "
                    f"{resume_errors}"
                )
            ...
            resume_missing = [
                k
                for k in resume_required
                if k not in resume_result
            ]
            ...
            if resume_missing:
                ...
                raise RuntimeError(
                    "Fold 1 resume result missing keys: "
                    f"{resume_missing}"
                )
            ...
            if (
                ...
                raise RuntimeError(
                    "Fold 1 frozen result metadata mismatch."
                )
            ...
            print()
            print(
                "[RESUME PASS] Fold 1 frozen result verified "
                "and restored without retraining "
                "| train=515 | val=99 | outer=351"
            )
