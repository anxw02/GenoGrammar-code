...
# GenoGrammar-v2: preregistered RNOE + CMOC
import sys as _ggv2_sys
_ggv2_sys.path.insert(
    0,
    str(__import__("pathlib").Path(__file__).resolve().parent)
)
import genogrammar_v2_cmoc_rnoe as v2
...
def build_fold_split(
    ...
    canonical_lineage_path = Path(
        r"/root/autodl-tmp/axw/01_1_ani_robustness/06_FINAL_robust_ANI_blocks.csv"
    )
...
def train_fold(
    ...
    model = (
        v2.make_v2_encoder(m.ExplicitNeighborhoodEncoder)
        .to(DEVICE)
    )
    ...
    for epoch in range(
        ...
        for step in range(
            ...
            with torch.autocast(
                ...
                loss, metrics = (
                    v2.forward_objectives_v2(m.forward_objectives,
                        model,
                        store,
                        fam,
                        strand,
